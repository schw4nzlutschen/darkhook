...
// Heap used by the library (hooks, trampolines, buffers)
...
static phook_entry add_hook_entry( )
{
    ...
    else if ( g_hooks.size >= g_hooks.capacity )
    {
        phook_entry p = (phook_entry)HeapReAlloc(
            GetProcessHeap(), 0, g_hooks.items, ( g_hooks.capacity * 2 ) * sizeof( hook_entry ) );
        if ( p == NULL )
            return NULL;
...
static DWORD_PTR find_old_ip( phook_entry hook, DWORD_PTR ip )
{
    ...
    for ( UINT i = 0; i < hook->instruction_count; ++i )
    {
...
static BOOL enumerate_threads( pfrozen_threads threads )
{
    ...
    if ( snapshot_handle != INVALID_HANDLE_VALUE )
    {
        ...
        if ( Thread32First( snapshot_handle, &te ) )
        {
            ...
            do
            {
                    ...
                    && te.th32ThreadID != GetCurrentThreadId( ) )
                {
                    if ( threads->items == NULL )
                    {
                        ...
                        threads->items
                            = (LPDWORD)HeapAlloc( GetProcessHeap(), 0, threads->capacity * sizeof( DWORD ) );
                        if ( threads->items == NULL )
                        {
                            succeeded = FALSE;
                            break;
                    ...
                    else if ( threads->size >= threads->capacity )
                    {
                        ...
                        p = (LPDWORD)HeapReAlloc(
                            GetProcessHeap(), 0, threads->items, threads->capacity * sizeof( DWORD ) );
                        if ( p == NULL )
                        {
                            succeeded = FALSE;
                            break;
            ...
            {
                HeapFree( GetProcessHeap(), 0, threads->items );
                threads->items = NULL;
            }
        }
...
static dh_status freeze_threads( pfrozen_threads threads, UINT pos, UINT action )
{
    ...
    for ( ;; )
    {
        ...
        for ( UINT i = 0; i < round.size; ++i )
        {
            ...
            if ( suspended )
            {
                // append to frozen list (process heap)
                if ( threads->items == NULL )
                {
                    threads->capacity = INITIAL_THREAD_CAPACITY;
                    threads->items = (LPDWORD)HeapAlloc(
                        GetProcessHeap(), 0, threads->capacity * sizeof( DWORD ) );
                    if ( threads->items == NULL )
                    {
                        status = DH_ERROR_MEMORY_ALLOC;
                        break;
                ...
                else if ( threads->size >= threads->capacity )
                {
                    ...
                    LPDWORD p = (LPDWORD)HeapReAlloc(
                        GetProcessHeap(), 0, threads->items,
        ...
        if ( round.items )
            HeapFree( GetProcessHeap(), 0, round.items );
...
dh_status WINAPI dh_initialize( VOID )
{
    ...
    {
        ReleaseSRWLockExclusive( &g_hooks_lock );
        return DH_ERROR_ALREADY_INITIALIZED;
    }
...
{
    // Phase 0: quick check under hooks lock
    AcquireSRWLockExclusive( &g_hooks_lock );
    ...
    // Phase 1: disable hooks OUTSIDE hooks lock
    dh_status status = enable_all_hooks_frozen( FALSE );
    if ( status != DH_OK )
        return status;

    // Phase 2: destroy resources under hooks lock
    AcquireSRWLockExclusive( &g_hooks_lock );
    ...
    LPVOID* original, LPVOID* target_out )
{
    ...
    LPVOID target = (LPVOID)GetProcAddress( module_handle, proc_name );
    if ( target == NULL )
...
#define DH_ST2STR(x)    \
    ...
    switch ( status ) {
        ...
        DH_ST2STR( DH_ERROR_INVALID_STATE )
    }
